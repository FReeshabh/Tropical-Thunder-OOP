getTotalReps(Card[] hand) {
  for(i = 0; i < 7; i++) {
    if(hand[i].setworkout == "Push Ups")
      {
        PushupReps += hand(i).getreps;
      }else 
      if(hand[i].setworkout == "Squats")
      {
        SquatReps += hand(i).getreps;
      }else
      if(hand[i].setworkout == "Sit Ups")
      {
        SitupReps += hand(i).getreps;
      }else
      if(hand[i].setworkout == "Lounges")
      {
...
getTotalRepSkipped(Card[] hand){
  for(i = 0; i < 7; i++) {
    if(hand[i].setworkout == "skip" && "BLUE")
      {
        PushupRepskipped += hand(i).getreps;
      }else 
      if(hand[i].setworkout == "Squats" && "YELLOW")
      {
        SquatRepskipped += hand(i).getreps;
      }else
      if(hand[i].setworkout == "Sit Ups" && "RED")
      {
        SitupRepskipped += hand(i).getreps;
      }else
      if(hand[i].setworkout == "Lounges" && "GREEN")
      {
